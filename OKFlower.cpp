...
int main (int argc, char** argv)
{
	if (argc != 4)
    {
    	...
    	cerr << "Typ: 9600 /dev/tty.usbmodemfd131 10\n";
        return 1;
    } 
	...
	try {
    	...
    	BufferedAsyncSerial serial(argv[2], boost::lexical_cast<unsigned int>(argv[1]));
	...
	while (!viewer->wasStopped ())
	{
		...
		if (BackgroundSub) {
			...
			printf("Cloud Size: %d vs. Max: %d vs. perc: %f\n", newPointIdxVector.size(), 640*480, (float)newPointIdxVector.size()/(float)(640.0*480.0));
			if (((float)newPointIdxVector.size()/(float)(640.0*480.0)) >= percentage) {
